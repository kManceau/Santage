            ...
            @foreach ($children as $child)
                <div class="col-md-4 mb-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <p class="my-0"><strong>Sexe : </strong>{{ $child->gender == "male" ? 'Garçon' : 'Fille' }}</p>
                            <p class="my-0"><strong>Age : </strong>{{ \Carbon\Carbon::parse($child->birthdate)->age }} ans</p>
                            <p class="my-0 mt-3"><strong>Adresse : </strong>{{ $child->address }}</p>
                            <p class="my-0"><strong>Code Postal : </strong>{{ $child->postal_code }} {{$child->city}}</p>
                            ...
                            @endphp
                                <p class="my-0 {{ $average > 10 ? 'text-success' : 'text-danger' }}"><strong>Moyenne des notes : </strong>{{ $average }}</p>
                            ...
                            @else
                                @foreach ($child->gift as $gift)
                                    <form action="{{ route('delete_gift_child', [$child->id, $gift->id]) }}" method="POST" class="mt-1">
                                        @csrf
                                        @method('DELETE')
                                        <p class="my-0 {{$gift->good ? 'text-success' : 'text-danger'}}">
                                            - {{$gift->name}}<button type="submit" style="background:none;border:none;" title="Delete">❌️</button>
                                        </p>
                                    </form>
                              @endforeach
                            @endif
                            <div class="mt-3">
                                <a href="{{ route('child_gift_add', $child->id) }}" class="text-decoration-none text-dark">🎁 Ajouter un cadeau</a>
                            </div>
                        </div>
                    </div>
                </div>
